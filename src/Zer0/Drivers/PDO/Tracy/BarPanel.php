...
namespace Zer0\Drivers\PDO\Tracy;

use Zer0\Drivers\PDO\PDO;

/**
 * Class BarPanel
 * @package Zer0\Drivers\PDO\Tracy
 */
class BarPanel implements \Tracy\IBarPanel
{
    ...
    public function getTab(): ?string
    {
        ...
        $html .= ' / ' . round(array_sum(array_column($queries, 'time')) * 1000) . ' ms';
        return $html;
    }
    ...
    public function getPanel(): ?string
    {
        ...
        if (count($queries) > 0) {
            ...
            foreach ($queries as $query) {
                ...
                if (class_exists('\SqlFormatter')) {
                    $html .= '<td>' . htmlspecialchars(\SqlFormatter::highlight($query['sql'])) . '</td>';
                } else {
                    $html .= '<td ' . $this->query_attributes . '>' . htmlspecialchars($query['sql']) . '</td>';
                }
                $html .= '</tr>';
            }
            $html .= '</table>';
